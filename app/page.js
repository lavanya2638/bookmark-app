...
export default function Home() {
  ...
  async function handleAddBookmark(e) {
    ...
    if (error) {
      ...
      // The real-time subscription will automatically add it to the list
      // But if realtime is slow, we can add it optimistically
      if (data && data[0]) {
        setBookmarks((current) => {
          const exists = current.some(b => b.id === data[0].id)
          if (exists) return current
  ...
  async function handleDeleteBookmark(id) {
    ...
    if (error) {
      ...
      // Refetch to restore the bookmark if delete failed
      fetchBookmarks()
    }
  }
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-6">
          ...
          <form onSubmit={handleAddBookmark} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full bg-blue-500 text-white px-6 py-3 rounded-lg font-medium hover:bg-blue-600 transition disabled:bg-gray-400 disabled:cursor-not-allowed"
            >
              ...
              {bookmarks.map((bookmark) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="ml-4 bg-red-100 text-red-600 px-3 py-1 rounded hover:bg-red-200 transition flex-shrink-0"
                  >
                    Delete
                  </button>
                </div>
